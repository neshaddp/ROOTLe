...
void SimpleBackgroundSubtractionTwoBars()
{
    bool wait = true;
	...
	TString detector2 = "NL03";

    TString Source_runnum = "5357";
    TString Background_runnum = "5359";
	
	TString Source_runnum2 = "5357";
    TString Background_runnum2 = "5359";

    TString hname = detector + "_AvgE";
	TString hname2 = detector2 + "_AvgE";

    double runtime_src = 1;
    double runtime_bg = 1;
	
    double runtime_src2 = 1;
    double runtime_bg2 = 1;
    ...
    if (!(f_src->GetListOfKeys()->Contains(hname2))) {
        std::cout << "ERROR: Can't find histogram " << hname2 << std::endl;
        exit(0);
    }

    TFile* f_bg2 = TFile::Open("histograms/HistogramsFromRun" + Background_runnum2 + ".root");
    if (!(f_bg->GetListOfKeys()->Contains(hname2))) {
        std::cout << "ERROR: Can't find histogram " << hname2 << std::endl;
        exit(0);
    }

    // Retrieve histograms
    TH1D* h_src2 = (TH1D*)f_src2->Get(hname2);
    h_src2->Sumw2();

    TH1D* h_bg2 = (TH1D*)f_bg2->Get(hname2);
    h_bg2->Sumw2();
	...
	h_src_scale->SetLineColor(kBlack); h_src_scale->SetLineWidth(2);
	h_bg_scale->SetLineColor(kBlue); h_bg_scale->SetLineWidth(2);
	h_subtracted->SetLineColor(kRed); h_subtracted->SetLineWidth(2);
    h_subtracted->Draw("HIST");
	h_src_scale->Draw("HIST SAME");
	h_bg_scale->Draw("HIST SAME");
	
	
	h_src_scale2->SetLineColor(kBlack); h_src_scale2->SetLineStyle(2); h_src_scale2->SetLineWidth(2);
	h_bg_scale2->SetLineColor(kBlue);   h_bg_scale2->SetLineStyle(2); h_bg_scale2->SetLineWidth(2);
	h_subtracted2->SetLineColor(kRed);  h_subtracted2->SetLineStyle(2); h_subtracted2->SetLineWidth(2);
	...
	legend->AddEntry(h_src_scale, "RMD Source Data (1 Hr)", "l");
    legend->AddEntry(h_bg_scale, "RMD Background Data (1 Hr)", "l");
    legend->AddEntry(h_subtracted, "RMD Subtracted Data (1 Hr)", "l");
	legend->AddEntry(h_src_scale2, "LENDA Source Data (1 Hr)", "l");
    legend->AddEntry(h_bg_scale2, "LENDA Background Data (1 Hr)", "l");
    legend->AddEntry(h_subtracted2, "LENDA Subtracted Data (1 Hr)", "l");
