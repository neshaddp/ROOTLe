...
int Run(Int_t runNum=-1,TString OutPutName="histograms/temp.root")
{
  ...
  if (runNum != -99){
    ...
    if (runNum ==-1){
	  ...
	  //ss<<"/mnt/analysis/e23009/Neshad_temp/run-"<<setfill('0')<<setw(4)<<runNum<<"-??.root";
	  //ss<<"/mnt/rawdata/e17018/rootfiles/run-"<<setfill('0')<<setw(4)<<runNum<<"-??.root";
	  ss<<"/mnt/analysis/e17018/rootfiles/run-"<<setfill('0')<<setw(4)<<runNum<<"-??.root";
      ch->Add(ss.str().c_str());
