...
public:
  ...
  void SetLeadingEdgeTimes(const vector<Double_t> & v){ rLETimes=v; }///<Sets the leaading-edge times for this channel
  ...
  /**Gets the leading-edge times for this channel. At present, it can only accomodate leading-edge of first pulse when there are many pulses in a traces
   */
  vector<Double_t> GetLeadingEdgeTimes()const { return rLETimes;}
  ...
  inline Double_t GetFilterEnergy(){return rFilterEnergy;}///<Gets energy from trapezoidal filter trace analysis
  ...
  inline Double_t GetLeadingEdgeTime(){return rLETimes[0];}///<Gets the offline timestamp from the leading-edge algorithm 
  ...
  // Add functions to extract external clock from LENDA channel. JP 1/26/2016
  inline UInt_t GetExClockLow(){return rExClockLow;}///<Low 32 bits of the external clock
  inline UInt_t GetExClockHigh(){return rExClockHigh;}///<High 16 bits (from bit 32 to bit 47) of the external clock
  inline Double_t GetExClock(){return rExClock;}///<Gets the external clock
  ...
  // Add functions to set external clock from ddaschannel info. JP 1/26/2016
  void SetExClockLow(UInt_t v){rExClockLow=v;}
  void SetExClockHigh(UInt_t v){rExClockHigh=v;}
  void SetExClock(Double_t v){ rExClock=v;}
  ...
  void SetFilterEnergy(Double_t v) {rFilterEnergy=v;}
...
private:
  ...
  // Added variables for external time. JP 1/25/2016
  UInt_t rExClockLow;
  UInt_t rExClockHigh;
  Double_t rExClock;
