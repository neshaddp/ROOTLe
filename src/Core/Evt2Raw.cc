...
int main(int argc, char* argv[])
{
   ...
   while ( !signal_received) {
     ...
     //from the file inorder for the eof bit to true
      ...
      if ( feof(infile) ){ // if the end of tile bit has been set
        if ( bytes_read != info.fSize){
          Error("Evt2Raw","Total number of bytes read %ld not equal to file size %lld at the end of file parsing",bytes_read,info.fSize);
        }
     ...
     ". Bytes read: " << bytes_read << endl;
      ...
      // The pointer should be pointing at the beginning of the main "body" of built ring item.
		  ...
		  case S800_SOURCEID:
		  {
			...
			case PHYSICS_EVENT:
			{
			  ...
			  // Check now selfinclusive body payload (16-bit event length)
			   ...
			   s800->SetTS((long long int)s800_tstamp);
