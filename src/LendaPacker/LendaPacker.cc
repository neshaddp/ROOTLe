...
void LendaPacker::CalcAll(vector<UShort_t>& theTrace,MapInfo info){
  ...
  if (theTrace.size()!=0 && info.DontTraceAnalyze==false){
    ...
    if (! info.IsAReferenceChannel ){
      ...
      //The filter energy is not implemented as a vector
      ...
      longGate=theFilter.GetGate(theTrace,MaxSpotInTrace_temp-4,30);
      filter_energy = theFilter.GetFilterEnergy(theTrace);
      ...
      thisChannelsLE.push_back(theFilter.GetLeadingEdge(theTrace)); //Leading-edge time. JP 5/18/2018
	...
	filter_energy = theFilter.GetFilterEnergy(theTrace);
	thisChannelsLE.push_back(theFilter.GetLeadingEdge(theTrace)); //There is not yet a LE version for high-rates
  ...
  if (thisChannelsLE.size()==0){ // added by JS 12/5/18
    thisChannelsLE.push_back(BAD_NUM);
  }
 ...
 void LendaPacker::BuildMaps(){
   ...
   int dontTrace;
     if (line[0] != '#' ){
       ...
       cout << "slot: " << slot << " ch: " << channel << " name: " << name << " angle: " << Angle <<
	 " ref: " << ReferenceName << " fl: " << FL << " fg: " << FG << " d: " << d << " w: " << w <<
	 " dontTrace: " << dontTrace << endl; 
...
void LendaPacker::UpdateSettings(){
 ...
 LendaChannel LendaPacker::DDASChannel2LendaChannel(ddaschannel* c,MapInfo info){
   ...
   // Extract external clock from ddaschannel c. JP 1/25/2016
   tempLenda.SetExClock(c->exclock);
   tempLenda.SetExClockLow(c->exclocklow);
   tempLenda.SetExClockHigh(c->exclockhigh);
 ...
 void LendaPacker::PackCalculatedValues(LendaChannel* theChannel,MapInfo & info){
   Double_t CorseTime =2*(theChannel->GetTimeLow() + theChannel->GetTimeHigh() * 4294967296.0); // CorseTime is
   // multiplied by 2 because the time-stamping clock is twice slower than the 250-MHz digitizing clock
   ...
   vector <Double_t> TempLeadingEdgeTimes;
   for (int i=0;i<(int)thisChannelsSoftwareCFDs.size();i++){
     ...
     TempLeadingEdgeTimes.push_back( CorseTime + thisChannelsLE[i]);
   }
 ...
 void LendaPacker::MakeLendaEvent(LendaEvent *Event,DDASEvent *theDDASEvent,
   ...
   for (int i=0;i<(int)theDDASChannels.size();i++){
     ...
     if ( it != GlobalIDToMapInfo.end()){ // the channel is in the map
       ...
       //cout << "------ DDAS channel: " << id << " LENDA channel name: " << fullName << endl;
	 ...
	 //The channels will be packed into the LendaEvent later below
