...
Double_t LendaFilter::GetZeroCrossingImprovedRMD(std::vector <Double_t> & CFD,Int_t& MaxInTrace,Double_t & residual){
  ...
  for (int j = begin;j < end;j++) {
    if (CFD[j] >= 0 && CFD[j+1] < 0 ) {
      //zero crossing point
      softwareCFD =j + CFD[j] / ( CFD[j] + TMath::Abs(CFD[j+1]) ); //A simple interpolation
      residual=CFD[j];
      break;
    }
  }
...
Double_t LendaFilter::GetZeroCubicRMD(std::vector <Double_t> & CFD,int &start){
  
  
  // std::map <double,int> zeroCrossings;
  // double max=0;
  Double_t zero = BAD_NUM;
  
  //int begin = MaxInTrace;
  int begin = start; // Changed by JP for RMD analysis, 05/13/2016
  //int end = (CFD.size()/2)+40;
  int end = CFD.size(); // Changed by JP for RMD analysis, 05/12/2016
  

  //cout << "start " << start << endl;

  if (end <= begin) {
    cout << "***** ERROR In GetZeroCubicRMD: end of CFD range smaller than beginning" << endl;
    return zero;
  }
  

  
  for (int j=start+5; j<end; j++) { // Ensure that the starting point is ok
    if ( (CFD[j]>0) && (CFD[j+1]>0) ) {
      begin = j;
      break;
    }
...
Double_t LendaFilter::GetEnergyRMD(std::vector <UShort_t> &trace,Int_t MaxSpot){ // JP for RMD 2017 campaign
...
Double_t LendaFilter::GetGateRMD(std::vector <UShort_t> &trace,int &start,int L){
...
Double_t LendaFilter::GetLeadingEdge(std::vector <UShort_t> &trace){


  int traceLength=trace.size();
  bool foundLE = false;
  Double_t letime = BAD_NUM;
  ...
  if (maxSpot < LengthForBackground) { // Make sure that pulse maximum is not in the baseline region
    //cout << "******* WARNING: Pulse max too early (in the baseline region) !!!" << endl;
    background = BAD_NUM;
    return letime;
  ...
  while( j<traceLength-1 && !foundLE ) {
    ...
    if (trace[j] < LeadingEdge && trace[j+1] > LeadingEdge) {

      letime = j + (LeadingEdge - trace[j]) / (trace[j+1] - trace[j]);
      foundLE = true;
...
Double_t LendaFilter::GetFilterEnergy(std::vector <UShort_t> &trace){
  ...
  if (S0begin > 0 && S1end < traceLength) {
    ...
    //cout << "******* WARNING: Bad energy filter parameters !!!" << S0begin << " " << S1end << " " << G/2 << endl;
    At = BAD_NUM;
  }
...
//Double_t LendaFilter::GetMaxPulseHeightBaseline(std::vector <UShort_t> &trace,Int_t MaxSpot) {
Double_t LendaFilter::GetMaxPulseHeightBaseline(std::vector <UShort_t> &trace,Int_t& MaxSpot) { // Pass MaxSpot as reference
  ...
  if (BackGround != BAD_NUM) {
    max_base = max - BackGround;
  } else {
    max_base = BAD_NUM;
  }

  //cout << "Max: " << max << "Background: " << BackGround << "Max-baseline: " << max_base << endl;
...
Double_t LendaFilter::GetPulseComplete(std::vector <UShort_t> &trace, Int_t &MaxSpot, Double_t &rising, Double_t &background, Double_t &noise) {

  // This function extracts several parameters characterizing the pulse shape (JP 5/20/2016)
  // Returns the pulse-height minus the baseline,
  // It also calculates: maximum, rising time, baseline, and noise level
  ...
  if (_maxSpot < LengthForBackground) { // Make sure that pulse maximum is not in the "noise" baseline region
    ...
    sigma2 = 0;
    for (int j=0 ;j<LengthForBackground;j++) { // Scan trace
      sigma2 = sigma2 + pow((trace[j] - _background),2);
      //sigma2 = abs(trace[j] - _background);
    }

    //cout << sumBackground << "   " << _background << "    " << sigma2 << endl;

    _noise = sqrt(sigma2/LengthForBackground);

    //if (min < max) { // Calculate noise
    // _noise = max - min;
    //} else {
    // _noise = BAD_NUM;
    //}
