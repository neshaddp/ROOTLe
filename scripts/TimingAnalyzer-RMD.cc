...
int main(int argc, char **argv){
  ...
  //TFile *outFile = new TFile("Result-test2017-set1.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD101.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD102.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD103.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD104.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD105.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD106.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD107.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD108.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD109.root","recreate");

  //TFile *outFile = new TFile("Result-test2017-RMD101-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD102-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD103-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD104-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD105-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD106-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD107-highE.root","recreate");
  //TFile *outFile = new TFile("Result-test2017-RMD108-highE.root","recreate");
  TFile *outFile = new TFile("Result-test2017-RMD109-highE.root","recreate");
  ...
  for (int FL=FL_low;FL<FL_high;FL++){ 
	...
	for (int d=d_low;d<d_high;d++){
	  ...
	  TheCubic_vs_Energy[count]=new TH2F(nameStream.str().c_str(),"Title",nBins,xlow,xhigh,ERMDBins,0,ERMDMax);
	  ...
	  TheNoise_vs_ERMDHistograms[count]=new TH2F(nameStream.str().c_str(),"Title",ERMDBins,0,ERMDMax,500,0,20);
	  ...
	  TheCubic_vs_SigNoise[count]=new TH2F(nameStream.str().c_str(),"Title",nBins,xlow,xhigh,500,0,200);
	  //TheCubic_vs_SigNoise[count]=new TH2F(nameStream.str().c_str(),"Title",1000,-25,-5,500,0,200);
  ...
  for (Long64_t jentry=0; jentry<nentry;jentry++) { // Main analysis loop. Go through all entries in the root file
    ...
    //cout << "Num bars: " << inEvent->NumBars << endl;
    //cout << "SimpleBit 0: " << inEvent->Bars[0].SimpleEventBit << endl;
    //cout << "SimpleBit 1: " << inEvent->Bars[1].SimpleEventBit << endl;
    //cout << "Bar 0 name: " << inEvent->Bars[0].Name << endl;
    //cout << "Bar 1 name: " << inEvent->Bars[1].Name << endl;
	...
	for (int FG=FG_low;FG<FG_high;FG++){
	  for (int w=w_low;w<w_high;w++){
	    for (int d=d_low;d<d_high;d++){
	      ...
	      //cout << "LENDA energy: " << lenda_E << endl;
	      ...
	      Double_t time = outEvent->Bars[1].Tops[0].GetTime() - lenda_time;
	      Double_t CubicTime = outEvent->Bars[1].Tops[0].GetCubicTime() - lenda_cubictime;
	      //Double_t E = outEvent->Bars[1].Tops[0].GetEnergy();
	      Double_t E = outEvent->Bars[1].Tops[0].GetPulseHeight();
	      //if (outEvent->Bars[1].Name=="RMD104") { // Calibration for RMD104 (2017 campagin)
	      //E = 0.7305 * E + 10.015;
	      //}
		...
		E = 0.6842 * E + 11.573;
	      ...
	      if (lenda_E > 5000 && E > 200) { // Include only high-E for both LENDA and RMD
		...
		LendaChannel lendach = outEvent->Bars[0].Tops[0];
		...
		TheNoiseLENDAHistograms[count]->Fill(Noise);
	      ...
	      //cout << "E: "<< E << " CubicTime: " << CubicTime << endl;
  ...
  TF1 * aGauss = new TF1("aGauss","gaus",-20,-4);
  ...
  //ofstream out("./TheResoultions-set1.txt");
  ofstream out("./TheResoultions-set2.txt");
