...
int main(int argc, char **argv){
  ...
  ////////////2////////////////////////////////////////////////////////////////////////
  TFile *outFile = new TFile("PSD-test.root","recreate");
 

 
  int shortgate_low = 50;
  int shortgate_high = 70;
  int longgate_low = 500;
  int longgate_high = 600;
  ...
  int NumberOfParameterSets = ((shortgate_high -shortgate_low)/20.)*((longgate_high - longgate_low)/100.);
  ...
  for (int rmd_index=0; rmd_index<NumberRMD; rmd_index++) { // Loop over all RMDs
	...
	nameStream.str("");
	nameStream<<"RMD"<<RMD<<"-SG"<<shgate<<"LG"<<lgate<<" PSD (400 keVee)";
  ...
  for (Long64_t jentry=0; jentry<nentry;jentry++) { // Main analysis loop. Go through all entries in the root file
    ...
    for (int ibar=0; ibar < NumRMD; ibar++) {
      ...
      //Double_t E = inEvent->Bars[0].Tops[0].GetEnergy();
	...
	E = -9990;
	  ...
	  if ((lG+sG) > 0) {
	    ...
	    if (E > 200) RMD_PSD1[RMDId][count]->Fill(psd);
	    if (E > 300) RMD_PSD2[RMDId][count]->Fill(psd);
	    if (E > 400) RMD_PSD3[RMDId][count]->Fill(psd);
  ...
  Double_t par[6];
  TF1 *aGauss1 = new TF1("aGauss1","gaus",0.70,0.76);
  TF1 *aGauss2 = new TF1("aGauss2","gaus",0.76,0.80);
  TF1 *aGauss = new TF1("aGauss","gauss(0)+gauss(3)",0.70,0.82);
